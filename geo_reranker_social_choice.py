...
def load_top_k(basedir, method):
    file_path = os.path.join(basedir, method, "top_k_recommendations.json")
    ...
    for dataset in tqdm(available_datasets, desc="Processing datasets"):
        ...
        for result in tqdm(
            ...
            try:
                ...
                _, basedir = recommender_dir_combiner(
                    dataset, result["directory"]
                )
        

                for aggregation in ["borda", "schulze"]:
                    agg_df = load_top_k(basedir, aggregation)

                    out_df = reranker.rerank_all(
                        recommendations_df=agg_df,
                        top_k=top_k_eval,
                    )
                    save_top_k(out_df, basedir, f"{aggregation}_geo")
